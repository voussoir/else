...
class LogoGame:
	...
	def gui_rebuild_game(self, *b):
		...
		for logo in self.all_logos:
			...
			entry_name = tkinter.Entry(logoframe, justify='center')
			if logo.solved is True:
				...
				label_image.configure(image=i)
				label_image.i = i
				entry_name.logo = logo
				entry_name.grid(row=1, column=0, sticky='s')
				entry_name.bind('<Return>', self.solve)
	...
	def solve(self, event):
		...
		guess = guess.lower()
		solutions = event.widget.logo.solutions
		if any(solution.lower() == guess for solution in solutions):
			event.widget.configure(state='disabled')
			self.playerstats_sethassolved(event.widget.logo.id)
			event.widget.logo.solved = True
	...
	def playerstats_gethassolved(self, logoid):
		key = 'hassolved_%d' % logoid
		val = self.playerstats_get(key)
		if val in [None, 0, False, '0']:
			return False
		return True

	def playerstats_sethassolved(self, logoid):
		key = 'hassolved_%d' % logoid
		val = 1
		self.playerstats_set(key, val)
